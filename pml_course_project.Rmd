...
## Load Data
```{r}
trainSet <- read.table("C:/Users/Sean/Desktop/Coursera/pml-training.csv", header=TRUE, sep=",")

testSet <- read.table("C:/Users/Sean/Desktop/Coursera/pml-testing.csv", header=TRUE, sep=",")
```
...
Training and validation datasets are created on a 60/40 basis.
...
The randomForest package was used as it can be more efficient than the Random Forest method in the caret package. A 10-fold cross validation was used as train control method. Here is the result of the model and the importance of each predictor
...
The out-of-sample error is the error realised by using the model developed on the training data to make predictions on separate validation sample. An estimate is that it should be close to the OOB estimate of error rate in the model. The cross validation shows the model to be very accurate, with an accuracy against the validation sample of 99.11%, with the out-of-sample error of 
...
# then, run the stacked model against the validation set
...
```{r}
library(elasticnet)
lassoModel<- train(classe ~ ., data=training, method="lasso")
## plot the results
plot.enet(lassoModel$finalModel, xvar = c( "penalty"), use.color = TRUE))
```

# Fit an SVM model to predict classe using the # default settings
``` {r}
library(e1071)
library(forecast)
...
acc <- accuracy(svmPred, validation$classe)

# pull out RMSE value
acc[2]
```
rfPred <- predict(rfModel, testSet, type = "class")
confusionMatrix(rfPred, validation$classe)